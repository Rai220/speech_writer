...
![](/_next/image/?url=%2Fimages%2Feth-gif-cat.png&w=1920&q=75)
...
## [what is identity permalink](\#what-is-identity) Что такое идентичность?
...
## [what are identifiers permalink](\#what-are-identifiers) Что такое идентификаторы?
...
## [benefits of decentralized identity permalink](\#benefits-of-decentralized-identity) Преимущества децентрализованной идентификации
...
5. Децентрализованная идентификация должна хорошо работать с новыми технологиями с нулевым разглашением, которые позволят людям доказать, что они владеют чем-то или сделали что-то, не раскрывая деталей. Это может стать мощным способом объединения доверия и конфиденциальности для различных приложений, например для голосований.

6. Децентрализованная идентификация позволяет механизмам противодействия атакам Сивиллы идентифицировать, когда один человек притворяется несколькими людьми, чтобы обмануть систему или распространить в ней спам.


## [decentralized identity use cases permalink](\#decentralized-identity-use-cases) Варианты использования децентрализованной идентификации

Децентрализованная идентификация имеет много потенциальных вариантов использования:

### [universal dapp logins permalink](\#universal-dapp-logins) 1\. Универсальные логины

Децентрализованная аутентификация может заменить вход по паролю. Поставщики услуг могут выдавать пользователям аттестаты, которые можно хранить в кошельке Ethereum. Примером аттестации может быть NFT, предоставляющий владельцу доступ к онлайн-сообществу.

Затем функция [Войти с помощью Ethereum(opens in a new tab)](https://login.xyz/) позволит серверам подтвердить учетную запись Ethereum пользователя и получить требуемое подтверждение с адреса его учетной записи. Это означает, что пользователи могут получать доступ к платформам и веб-сайтам, не запоминая длинные пароли, и улучшает работу пользователей в Интернете.

### [kyc authentication permalink](\#kyc-authentication) 2\. Аутентификация KYC
...
Децентрализованная идентификация позволяет компаниям отказаться от традиционных процессов [«знай своего клиента» (KYC)(opens in a new tab)](https://en.wikipedia.org/wiki/Know_your_customer) и аутентифицировать личность пользователя с помощью проверяемых учетных данных. Это снижает стоимость управления идентификацией и предотвращает использование поддельной документации.

### [voting and online communities permalink](\#voting-and-online-communities) 3\. Голосование и онлайн-сообщества
...
### [what are decentralized identifiers permalink](\#what-are-decentralized-identifiers) Что такое децентрализованные идентификаторы?
...
Децентрализованные идентификаторы хранятся в распределенных реестрах (блокчейнах) или одноранговых сетях. Это делает DID [глобально уникальными, разрешаемыми с высокой доступностью и криптографически верифицируемыми(opens in a new tab)](https://w3c-ccg.github.io/did-primer/). Децентрализованный идентификатор может быть связан с различными объектами, включая людей, организации или государственные учреждения.

## [what makes decentralized identifiers possible permalink](\#what-makes-decentralized-identifiers-possible) Что делает возможными децентрализованные идентификаторы?

### [public key cryptography permalink](\#public-key-cryptography) 1\. Криптография с открытым ключом

Криптография с открытым ключом — это средство защиты информации, которое генерирует открытый и  секретный ключи для участника. Криптография с открытым ключом используется в сетях блокчейн для аутентификации пользователей и подтверждения права собственности на цифровые активы.

Некоторые децентрализованные идентификаторы, такие как учетная запись Ethereum, имеют открытый и закрытый ключи. Открытый ключ идентифицирует контроллер учетной записи, а закрытые ключи могут подписывать и расшифровывать сообщения для этой учетной записи. Криптография с открытым ключом предоставляет доказательства, необходимые для аутентификации участников и предотвращения выдачи себя за другое лицо и использования поддельных идентификаторов, используя [криптографические подписи(opens in a new tab)](https://andersbrownworth.com/blockchain/public-private-keys/) для проверки всех утверждений.

### [decentralized datastores permalink](\#decentralized-datastores) 2\. Децентрализованные хранилища данных
...
## [how decentralized identifiers and attestations enable decentralized identity permalink](\#how-decentralized-identifiers-and-attestations-enable-decentralized-identity) Как децентрализованные идентификаторы и аттестации обеспечивают децентрализованную идентификацию?
...
В контексте децентрализованной идентификации аттестации (известные также как [проверяемые учетные данные(opens in a new tab)](https://www.w3.org/TR/vc-data-model/)) являются защищенными от несанкционированного доступа криптографически проверяемыми заявлениями эмитента. Каждая аттестация (проверяемые учетные данные), которую субъект (например, организация) выдает, связана с их DID.
...
## [types of attestations in decentralized identity permalink](\#types-of-attestations-in-decentralized-identity) Виды аттестаций в децентрализованной идентификации
...
### [off chain attestations permalink](\#off-chain-attestations) Аттестации вне сети
...
Решение состоит в выдаче аттестаций, хранящихся пользователями вне сети в цифровых кошельках, но подписанных с помощью DID эмитента, хранящегося в сети. Эти аттестации кодируются как [веб-токены JSON(opens in a new tab)](https://en.wikipedia.org/wiki/JSON_Web_Token) и содержат цифровую подпись эмитента, что позволяет легко проверять заявления вне сети.
...
### [offchain attestations with persistent access permalink](\#offchain-attestations-with-persistent-access) Аттестации вне сети с постоянным доступом

В этом случае аттестации преобразуются в файлы JSON и хранятся вне сети (в идеале на платформе [децентрализованного облачного хранилища](/ru/developers/docs/storage/), например IPFS или Swarm). Однако хэш файла JSON хранится в сети и связан с DID через реестр в сети. Связанный DID может принадлежать либо издателю аттестации, либо получателю.
...
### [onchain attestations permalink](\#onchain-attestations) Аттестации в сети
...
### [soulbound permalink](\#soulbound) Токены Soulbound и личность

[Токены Soulbound(opens in a new tab)](https://vitalik.eth.limo/general/2022/01/26/soulbound.html) (непередаваемые NFT) можно использовать для сбора информации, уникальной для конкретного кошелька. Это эффективно создает уникальную идентификацию в сети, привязанную к конкретному адресу Ethereum, которая может включать токены, представляющие достижения (например, окончание определенного онлайн-курса или достижение порогового балла в игре) или участие в сообществе.

## [use decentralized identity permalink](\#use-decentralized-identity) Используйте децентрализованную идентификацию

Есть много амбициозных проектов, использующих Ethereum в качестве основы для децентрализованной идентификации:

- **[Служба имен Ethereum (ENS)(opens in a new tab)](https://ens.domains/)** — _децентрализованная система имен для сетевых, машиночитаемых идентификаторов, таких как адреса кошельков Ethereum, хэши контента и метаданные._
- **[SpruceID(opens in a new tab)](https://www.spruceid.com/)** — _проект децентрализованной идентификации, который позволяет пользователям контролировать цифровую идентификацию с помощью учетных записей Ethereum и профилей ENS, а не полагаться на сторонние службы._
- **[Ethereum Attestation Service (EAS)(opens in a new tab)](https://attest.sh/)** — _децентрализованный протокол и перепись для аттестаций любого назначения в цепочке и вне ее._
- **[Proof of Humanity(opens in a new tab)](https://www.proofofhumanity.id)** – _«доказательство человечества» (PoH) – это социальная система подтверждения личности, построенная на Ethereum._
- **[BrightID(opens in a new tab)](https://www.brightid.org/)** – _децентрализованная сеть социальной идентификации с открытым исходным кодом, стремящаяся реформировать проверку личности путем создания и анализа социального графа._
- **[walt.id(opens in a new tab)](https://walt.id)** — _инфраструктура децентрализованной идентификации и кошелька с открытым исходным кодом, которая позволяет разработчикам и организациям использовать суверенную идентичность и NFT/SBT._
- **[Veramo(opens in a new tab)](https://veramo.io/)** — _фреймворк JavaScript, который упрощает использование криптографически проверяемых данных в приложениях._

## [further reading permalink](\#further-reading) Дополнительные ресурсы

### [articles permalink](\#articles) Статьи

- [Примеры использования блокчейна: блокчейн в цифровой идентификации(opens in a new tab)](https://consensys.net/blockchain-use-cases/digital-identity/) — _ConsenSys_
- [Что такое Ethereum ERC725? Самостоятельное управление идентификацией в блокчейне(opens in a new tab)](https://cryptoslate.com/what-is-erc725-self-sovereign-identity-management-on-the-blockchain/) — _Sam Town_
- [Как блокчейн может решить проблему цифровой идентификации?(opens in a new tab)](https://time.com/6142810/proof-of-humanity/) — _Эндрю Р. Чоу_
- [Что такое децентрализованная идентификация и почему это должно волновать вас?(opens in a new tab)](https://web3.hashnode.com/what-is-decentralized-identity) — _Эммануэль Авосика_
- [Введение в децентрализованные идентификаторы(opens in a new tab)](https://walt.id/white-paper/digital-identity) — _Доминик Берон_

### [videos permalink](\#videos) Видео

- [Децентрализованная идентификация (бонусная стрим-сессия)(opens in a new tab)](https://www.youtube.com/watch?v=ySHNB1za_SE&t=539s) — _отличное видео о децентрализованной идентификации от Андреаса Антонополуса_
- [Войти в систему с помощью Ethereum и децентрализованной идентификации с помощью Ceramic, IDX, React, и 3ID Connect(opens in a new tab)](https://www.youtube.com/watch?v=t9gWZYJxk7c) — _Учебное пособие на YouTube по созданию системы управления идентификацией для создания, чтения и обновления профиля пользователя с использованием его кошелька Ethereum от Надер Дебит_
- [BrightID — децентрализованная идентификация на Ethereum(opens in a new tab)](https://www.youtube.com/watch?v=D3DbMFYGRoM) — _подкаст бесплатных версий, обсуждающий BrightID, децентрализованное решение для Ethereum_
- [Презентация «Интернет вне сети»: децентрализованная идентификация и проверяемые полномочия(opens in a new tab)](https://www.youtube.com/watch?v=EZ_Bb6j87mg) — EthDenver 2022 от Эвина МакМаллена
- [Обьяснение проверяемых учетных данных(opens in a new tab)](https://www.youtube.com/watch?v=ce1IdSr-Kig) — видео на YouTube с демонстрацией Тамино Бауманна

### [communities permalink](\#communities) Сообщества

- [ERC-725 Альянс на GitHab(opens in a new tab)](https://github.com/erc725alliance) — _сторонники стандарта ERC725 для управления идентификацией на блокчейне Ethereum_
- [Сервер SpruceID Discord(opens in a new tab)](https://discord.com/invite/Sf9tSFzrnt) — _сообщество для энтузиастов и разработчиков, работающих над регистрацией в Ethereum_
- [Veramo Labs(opens in a new tab)](https://discord.gg/sYBUXpACh4) — _сообщество разработчиков, содействующих созданию фреймворка для проверяемых данных для приложений_
- [walt.id(opens in a new tab)](https://discord.com/invite/AW8AgqJthZ) — _сообщество разработчиков, работающих над вариантами использования децентрализованных идентификаторов в различных отраслях_